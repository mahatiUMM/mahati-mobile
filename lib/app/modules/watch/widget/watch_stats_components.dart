...
class WatchStatsComponent extends StatelessWidget {
  const WatchStatsComponent({
    super.key,
      ...
      child: StreamBuilder<List<int>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            if (isMi) {
              statusData = WatchUtils.handleMiSteps(values);
            } else {
              statusData = WatchUtils.getSteps(values);
            }
            return Row(
              children: [
                Expanded(
          ...
          if (isMi && miReadData != null) {
            var statusData = <String, int>{};
            statusData = WatchUtils.handleMiSteps(miReadData!);
            return Row(
              children: [
                Expanded(
                  child: Center(
